...
#pragma omp parallel for private(resid,j,i) reduction(+:error) // nowait  collapse(2) 
      for (i=2;i<(n-2);i++)  
        for (j=2;j<(m-2);j++)   
        {
          ...
          t41 = (t4 - t39) * t36;
          t43 = (t37 - t41) * t36;
          u[i][j] = (t13 - dx * ((((uold[i + 2][j] - t3) * t6 
          - t7) * t6 - t13) * t6 - (t13 - (t11 - (t9 - uold[i - 2][j]) * t6) * t6
          ) * t6) / 0.12E2 + t43 - dy * ((((uold[i][j + 2] - t34) * t36 - t37) 
          * t36 - t43) * t36 - (t43 - (t41 - (t39 - uold[i][j - 2]) * t36) * t36
          ) * t36) / 0.12E2 - alpha * t4) / (-0.5E1 / 0.2E1 / t66 - 0.5E1 / 
          0.2E1 / t69 - alpha);
          resid = uold[i][j] - omega * resid;  
