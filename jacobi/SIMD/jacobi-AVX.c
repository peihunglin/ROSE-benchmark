...
#pragma omp target data map(in:n, m, omega, ax, ay, b, f[0:n][0:m]) map(inout:u[0:n][0:m]) map(alloc:uold[0:n][0:m])
  ...
  {
    error = 0.0;    
    error_ps = _mm256_broadcast_ss(&error);
...
#pragma omp parallel for private(j,i)
      ...
      omega_ps = _mm256_broadcast_ss(&omega);
      ax_ps = _mm256_broadcast_ss(&ax);
      ay_ps = _mm256_broadcast_ss(&ay);
      b_ps = _mm256_broadcast_ss(&b);
#pragma omp target //map(in:n, m, omega, ax, ay, b, f[0:n][0:m], uold[0:n][0:m]) map(out:u[0:n][0:m])
#pragma omp parallel for private(resid,j,i,dl,dr,dt,db,f_ps,u_ps,error_ps) reduction(+:error) // nowait  collapse(2) 
      ...
      { 
        for (j=1;j<(m-1);j+=8)   
        {
          ...
          dt = _mm256_loadu_ps((float*)(uold[i-1]+j)); 
          db = _mm256_loadu_ps((float*)(uold[i+1]+j)); 
          dr = _mm256_loadu_ps((float*)(uold[i]+j+1)); 
          dl = _mm256_loadu_ps((float*)(uold[i]+j-1)); 
