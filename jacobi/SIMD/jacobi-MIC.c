...
#pragma omp parallel for private(resid,j,i,dl,dr,dt,db,f_ps,u_ps,error_ps) reduction(+:error) // nowait  collapse(2) 
      ...
      { 
        for (j=1;j<(m-1);j+=16)   
        {
          f_ps   = _mm512_loadunpacklo_ps(f_ps  ,(float*)(f[i]+j)); 
          f_ps   = _mm512_loadunpackhi_ps(f_ps  ,(float*)(f[i]+j+16)); 
          d    = _mm512_loadunpacklo_ps(d   ,(float*)(uold[i]+j)); 
          d    = _mm512_loadunpackhi_ps(d   ,(float*)(uold[i]+j+16)); 
          dt   = _mm512_loadunpacklo_ps(dt  ,(float*)(uold[i-1]+j)); 
          dt   = _mm512_loadunpackhi_ps(dt  ,(float*)(uold[i-1]+j+16)); 
          db   = _mm512_loadunpacklo_ps(db  ,(float*)(uold[i+1]+j)); 
          db   = _mm512_loadunpackhi_ps(db  ,(float*)(uold[i+1]+j+16)); 
          dr   = _mm512_loadunpacklo_ps(dr  ,(float*)(uold[i]+j+1)); 
          dr   = _mm512_loadunpackhi_ps(dr  ,(float*)(uold[i]+j+1+16)); 
          dl   = _mm512_loadunpacklo_ps(dl  ,(float*)(uold[i]+j-1)); 
          dl   = _mm512_loadunpackhi_ps(dl  ,(float*)(uold[i]+j-1+16)); 
//          resid = (ax*(uold[i-1][j] + uold[i+1][j])\
              + ay*(uold[i][j-1] + uold[i][j+1])+ b * uold[i][j] - f[i][j])/b;  
...
//          error = error + resid*resid ;   
          ...
          _mm512_packstorelo_ps((float*)(u[i]+j),u_ps);
//          _mm512_packstorehi_ps((float*)(u[i]+j)+64,u_ps);
#if 0
         int ii;
         printf("%d %d value:\t",i,j);
         for(ii = 0 ; ii < 16; ii++)
         {
           printf("%f ",*((float*)(&(u_ps))+ii));
         }   
         printf("\n");
